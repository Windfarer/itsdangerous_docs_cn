...
签名会被加在字符串尾部，中间由句号 (``.``)分隔。验证字符串，使用 :meth:`~Signer.unsign` 方法：
...
如果你想要可以过期的签名，可以使用 :class:`TimestampSigner` 类，它会加入时间戳信息并签名。在反签名时，你可以验证时间戳有没有过期：
...
itsdangerous目前只提供HMAC SHA的派生算法以及不使用算法，不支持基于ECC的算法。header中的算法将与序列化器中的进行核对，如果不匹配，将引发 :exc:`BadSignature` 异常。
